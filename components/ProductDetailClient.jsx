...
export default function ProductDetailClient({ product }) {
  ...
  const groupedVariants = Object.values(
    detailedProduct.variants.reduce((acc, v) => {
      if (!acc[v.size]) {
        acc[v.size] = {
          size: v.size,
          items: [],
  ...
  const productMedia =
    detailedProduct.media?.map((m) => m.media).filter(Boolean) ?? [];
  ...
  // 3. fallback
  if (mediaList.length === 0 && detailedProduct.main_media) {
    mediaList.push(detailedProduct.main_media);
  }
...
const deliveryEnd = endDate.toLocaleDateString("en-IN", {
  ...
  const isAvailable =
    ...
    (selectedItem ? selectedItem.is_available : true);
  ...
  touch-action: pan-y;
      ...
      >
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Ratings */}
          {detailedProduct.rating_count > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span
                    key={i}
                    className={
                      i < Math.round(detailedProduct.average_rating)
                        ? "text-amber-400"
                        : "text-gray-200"
                    }
                  >
              ...
              <span className="text-sm font-bold text-gray-700">
                {detailedProduct.average_rating}
              </span>
              <span className="text-sm text-gray-400">
                ({detailedProduct.rating_count} reviews)
              </span>
            </div>
          )}
  ...
  Estimated Delivery: {deliveryStart} - {deliveryEnd}
          ...
          <p className="text-sm text-gray-500">
            Category:{" "}
            <a
              href={`/category/${detailedProduct.product_category}`}
              className="text-teal-600 font-bold hover:underline capitalize"
            >
              {detailedProduct.product_category}
            </a>
          </p>
