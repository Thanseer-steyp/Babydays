...
useEffect(() => {
  const deals = productsData.filter((p) => {
    ...
    const mrp = Number(p.lowest_variant_mrp ?? p.mrp ?? 0);
    return mrp > price && price > 0;
  });

  deals.sort((a, b) => {
    const discA =
      (Number(a.lowest_variant_mrp ?? a.mrp ?? 0) -
        Number(a.lowest_variant_price ?? a.price ?? 0)) /
      (Number(a.lowest_variant_mrp ?? a.mrp) || 1);

    const discB =
      (Number(b.lowest_variant_mrp ?? b.mrp ?? 0) -
        Number(b.lowest_variant_price ?? b.price ?? 0)) /
      (Number(b.lowest_variant_mrp ?? b.mrp) || 1);
